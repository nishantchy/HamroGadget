...
const Search = () => {
  ...
  const getProductList = async () => {
    ...
    try {
      ...
      const filteredProducts = responseJson.message.filter((product) =>
  ...
  return (
    <div className="max-w-[1240px] w-full mx-auto py-11">
      ...
      {products.length === 0 ? (
        <p>No products found for the search term "{term}".</p>
      ) : (
        <div className="flex justify-evenly items-center gap-6 flex-wrap py-11">
          {products.map((product) => (
            <div
              key={product.id}
              className="w-72 h-96 bg-white rounded-lg shadow-md overflow-hidden"
            >
              <div className="h-48">
                <img
